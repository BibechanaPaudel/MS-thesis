...
```{r}
rep<-read.csv("Data-obj2-New/At_PGPR_New_Combine.csv", na.strings="na")
head(rep)
str(rep)
summary(rep)
```
...
SA <- ggarrange(left_col, right_col,
                    ncol = 2, widths = c(1, 1.75), common.legend = TRUE)
...
row1 <- annotate_figure(
  ggarrange(empty, OPR3, empty, nrow = 3, heights = c(1, 1.5, 1)),
  top = text_grob("JA biosynthesis", face = "bold", size = 15,color="darkred")
)
ja_genes <- c("PR4","PDF1.2","Thi2.1")
...
```{r}
separator <- ggplot() + # Create a separator plot (a thin horizontal gray line between SA and JA sections)
  geom_hline(yintercept = 0, color = "gray40", linewidth = 0.6) +
  theme_void() + # remove all plot elements
  coord_cartesian(ylim = c(-1, 1)) 
SA <- SA +
  labs(x = NULL, y = NULL) + # remove axis labels
  theme(axis.title = element_blank()) # remove axis title elements
...
combined_plot <- ggarrange(
  ...
  labels = c("A","", "B"),
  align = "v",
  heights = c(1,0.05, 1) # vertically align plots
)
