...
## Load the data
```{r}
rep <- read.csv("Data/Obj3/At_PGPR_CMV_Combine.csv", header=T)
...
run_lsd_per_dpi <- function(dpi_label) {
  ...
  lsd <- LSD.test(fit, "Treatment", p.adj = "none", group = TRUE, alpha=0.05)  # Fisher's LSD
...
```{r}
CMV <- rep %>%  # Start with the dataset `rep` and pipe (%>%) it into the next operations
...
```{r}
CMV$Leaf_sample <- ifelse(
  grepl("^IL", CMV$Dpi), "Inoculated",  # Matches Dpi values starting with IL
  ifelse(grepl("^SL", CMV$Dpi), "Systemic", NA)
)
```


```{r}
inoculated<-filter(CMV, Dpi=="IL-1" | Dpi=="IL-4" | Dpi=="IL-7" | Dpi=="IL-10") #Subset the CMV1 data to include only specific timepoints labeled "IL-1", "IL-4", "IL-7", and "IL-10".
...
geom_text(aes(y = avg.virus + se + offset, label = groups),position = position_dodge(width = 0.7), vjust = -0.5,size=4.5)+
  ...
  scale_fill_manual(values=cbbPalette,                    labels=c(expression(bold("Control")),expression(bolditalic("P. fluorescens")),expression(bolditalic("S.marcescens")),expression(bolditalic("B. subtilis"))))+
  scale_y_continuous(
...
```{r}
systemic<-filter(CMV, Dpi=="SL-7" | Dpi=="SL-10" | Dpi=="SL-14")
...
```{r, Combine fig}
CMV.combine<-ggarrange(CMV_IL, CMV_SL,    #combine the  figures 
                   nrow=1,
                   ncol=2,
                   common.legend=T,    # Gives the common legend for both figures
                   widths=c(1.25,1))   # Makes the width of 1st fig 1.25 times more than 2nd fig
CMV.combine
```
